...
from dataset.utils.molecule_feature_utils import get_atoms_3d_coordinates, get_atomic_charges
# from dataset.utils.molecule_feature_utils import get_atoms_distance_stats
from dataset.convert_dataset import sdf_to_rdkitmol
...
class AtomFeaturizer(Featurizer):
	...
	def atom_type(
			self, atom, allowable_set=None, include_unknown_set=True,
			return_dim=False
	):
...
# 			  use_chirality: bool = False,
			...
			# @todo: atom_char_settings uses seetings from coords_settings, so
			# it needs coords_settings to be set as well.
			atom_char_settings={ # Only works if use_atomic_charges == True
				'gaussian_method': 'm062x',
				'scaling': 'auto',
				},
			return_format: str = 'tf',
			**kwargs
			):
...
# 		self.use_chirality = use_chirality
		...
		self.use_atomic_charges = use_atomic_charges
		self.add_self_loop = add_self_loop
		self.return_format = return_format

		# Set atom and edge featurizers.
		if af_allowable_sets is None:
			...
			if add_Hs or add_3d_coords:
				af_allowable_sets['atom_type'] = [
					'C', 'N', 'O', 'F', 'P', 'S', 'Cl', 'Br', 'I', 'H'
				]
		else:
			if add_Hs or add_3d_coords:
				if 'H' not in af_allowable_sets['atom_type']:
					raise ValueError(
						'"add_Hs" or "add_3d_coords" is set to True, '
						'af_allowable_sets[\'atom_type\'] must contain "H".'
					)
		...
		if self.use_edges:
			...
			self.bond_featurizer = BondFeaturizer(
				allowable_sets=bf_allowable_sets, add_self_loop=add_self_loop)


# 		self.use_distance_stats = use_distance_stats
		self.add_Hs = add_Hs
		self.add_3d_coords = add_3d_coords
		self.coords_settings = coords_settings
		self.atom_char_settings = atom_char_settings
...
# 			   log_every_n=1000,
		...
		# Initialize graphs
		atom_features_list = []
		if self.add_3d_coords:
			atom_coords_list = []
		if self.use_atomic_charges != 'none':
			atomic_charges_list = []
		bond_features_list = []
		pair_indices_list = []

		# Construct graphs and compute features.
		mol_indices = kwargs.get('mol_indices', range(len(smiles_list)))
		for i_mol, smiles in zip(mol_indices, smiles_list):
			if self.add_3d_coords and self.use_atomic_charges != 'none':
				atom_features, atom_coords, atomic_charges, bond_features, pair_indices = self._featurize(smiles, i_mol=i_mol+1)
				atom_coords_list.append(atom_coords)
				atomic_charges_list.append(atomic_charges)
			elif self.add_3d_coords:
				atom_features, atom_coords, bond_features, pair_indices = self._featurize(smiles, i_mol=i_mol+1)
				atom_coords_list.append(atom_coords)
			elif self.use_atomic_charges != 'none':
				atom_features, atomic_charges, bond_features, pair_indices = self._featurize(smiles, i_mol=i_mol+1)
				atomic_charges_list.append(atomic_charges)
			else:
		...
		if self.use_atomic_charges != 'none':
			# Scale atomic charges.
			if self.atom_char_settings['scaling'] == 'auto':
				# in "auto" mode, the scaler will be set to `MinMaxScaler()`
				# if it is not given in method argument.
				from sklearn.preprocessing import MinMaxScaler
				self.acharges_scaler = kwargs.get(
					'acharges_scaler', MinMaxScaler(feature_range=(0, 1))
					)
				self.scale_atomic_charges(
					atomic_charges_list, self.acharges_scaler)
			else:
				raise ValueError(
					'Atomic charges scaling mode: "%s" can not be recognized. '
					'Possible candidates include "auto".' %
					self.atom_char_settings['scaling'])

			# Append atomic charges at the end of atom_features.
			for i, af in enumerate(atom_features_list):
				atom_features_list[i] = np.concatenate(
					(af, atomic_charges_list[i]), axis=1)


		# Convert graphs to desired format.
		if self.return_format == 'tf':
			# Convert lists to ragged tensors for tf.data.Dataset later on
			import tensorflow as tf
			if self.add_3d_coords:
				features = (
					tf.ragged.constant(atom_features_list, dtype=tf.float32),
					tf.ragged.constant(atom_coords_list, dtype=tf.float32),
			...
			# Convert lists to nupmy object array.
			if self.add_3d_coords:
				features = (
					np.array(atom_features_list, dtype=object),
					np.array(atom_coords_list, dtype=object),
					np.array(bond_features_list, dtype=object),
					np.array(pair_indices_list, dtype=object),
				)
			else:
				features = (
					np.array(atom_features_list, dtype=object),
			...
			raise ValueError('I do not understand the return_format you give me. :(')
...
# 				feat.edge_features = feat.edge_features.astype(int)
	...
	def _featurize(self, smiles: str, **kwargs) -> tuple:
		...
		"""
		# Compute or load 3d coordinates.
		if self.add_3d_coords or self.use_atomic_charges != 'none':
			coords, sdf_str = get_atoms_3d_coordinates(smiles, **self.coords_settings, **kwargs)
			molecule = sdf_to_rdkitmol(sdf_str, add_Hs=True, assign_stereo=True)
			if self.use_atomic_charges != 'none':
				atomic_charges = get_atomic_charges(
					# @todo: make sure it is compatible with the 3d coords return mode.
					atom_char_return_mode = self.use_atomic_charges,
					**self.coords_settings, **self.atom_char_settings,
					**kwargs)
		else:
		...
		if self.add_3d_coords and self.use_atomic_charges != 'none':
			return atom_features, coords, atomic_charges, bond_features, pair_indices
		elif self.add_3d_coords:
			return atom_features, coords, bond_features, pair_indices
		elif self.use_atomic_charges != 'none':
			return atom_features, atomic_charges, bond_features, pair_indices
		else:
	...
	def scale_atomic_charges(self, features, scaler):
		nb_features = 1
		...
		try:
			...
			for charges in features:
				feats_real = np.concatenate(
					(feats_real, charges), axis=0)
			# Fit the scaler.
...
# 				cur_max = feats_real.min(axis=0)
		...
		# Transform data.
		for i, charges in enumerate(features):
 			feats_real_fitted = scaler.transform(charges)
 			features[i] = feats_real_fitted
