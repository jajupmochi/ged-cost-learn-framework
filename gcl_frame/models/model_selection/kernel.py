...
def evaluate_gram_matrix(
	...
	if model == 'gram':
		if mode == 'reg':
			from sklearn.kernel_ridge import KernelRidge
			from gcl_frame.utils.distances import rmse
			estimator = KernelRidge(kernel='precomputed')
			# scoring = 'neg_root_mean_squared_error'
			perf_eval = rmse
		elif mode == 'classif':
			from sklearn.svm import SVC
			from gcl_frame.utils.distances import accuracy
...
# 		param_grid=None, fit_test=False, model='gram', mode='reg'
# ):
# 	# if the precomputed matrix is a Gram matrix:
# 	if model == 'gram':
# 		if mode == 'reg':
...
# 			param_grid=param_grid,
# 			fit_test=fit_test, model='gram', mode=model_type
# 		)
#
